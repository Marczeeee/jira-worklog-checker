...
def process_daily_worklog_check(dates, user, worklogs, config):
	wlogs_dict = {}
	for date in dates:
...
def process_user(username, date_range, config):
	...
	process_daily_worklog_check(date_range, user, worklogs, config)
...
for user in users:
	...
	if today.weekday()<5:
		...
		process_user(user, dates, config)
	else:
